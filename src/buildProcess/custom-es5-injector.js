// packages are imported by polymer-build
const dom5 = require('dom5/lib/index-next');
const parse5 = require('parse5');
const url = require('url');

const File = require('vinyl');
const {AsyncTransformStream, getFileContents} = require('polymer-build/lib/streams');
...
const webcomponentsLoaderMatcher = dom5.predicates.AND(
	dom5.predicates.hasTagName('script'), attrValueMatches('src', webcomponentsLoaderRegex));
 ...
 */
class CustomElementsEs5AdapterInjector extends AsyncTransformStream {
	constructor() {
		...
		return __asyncGenerator(this, arguments, function* _transformIter_1() {
			try {
				for (var files_1 = __asyncValues(files), files_1_1; files_1_1 = yield __await(files_1.next()), !files_1_1.done;) {
					...
					const contents = yield __await(getFileContents(file));
...
<script>window.customElements && document.write('<script type="text/javascript" src="${adapterScriptUrl}"></'+'script>')</script>
`);
...
module.exports = CustomElementsEs5AdapterInjector;